...
estados_t construir_edificio(Jugador* jugador, ABB<Edificio *>& arbol, Mapa*& mapa){
    ...
    if(((st = verificar_materiales(jugador,edificio, resta_material)) != ST_OK)) return st;

    if((st = obtener_coordenadas(mapa, fila, columna)) != ST_OK) return st;

    if((st = mapa->obtener_casillero(fila, columna)->verificar_condiciones_construccion(jugador->obtener_caracter())) != ST_OK) return st;
    ...
    cout << endl << MSJ_CONSTRUICCION_EXITOSA << endl;
    return st;
}

void mostrar_edificios_construidos(Jugador* jugador)
{
    cout << endl << MSJ_EDIFICIOS_CONSTRUIDOS_JUGADOR << endl;
...
estados_t demoler_edificio(Jugador* jugador, ABB<Edificio *>& arbol, Mapa*& mapa){
    ...
    if((st = mapa->obtener_casillero(fila, columna)->verificar_condiciones_demolicion(jugador->obtener_caracter())) != ST_OK) return st;
    ...
    jugador->decrementar_energia(costo_energia);
    cout << endl << MSJ_DEMOLICION_EXITOSA << endl;
    return st;
}
...
estados_t atacar_edificio(Jugador* jugador, Jugador* jugador_uno, Jugador* jugador_dos, ABB<Edificio *>& arbol, Mapa*& mapa){
    ...
    if((st = mapa->obtener_casillero(fila ,columna)->verificar_condiciones_ataque(jugador->obtener_caracter())) != ST_OK) return st;
    ...
    jugador->decrementar_energia(costo_energia);
    cout << endl << MSJ_ATAQUE_EXITOSO << endl;
    return st;
}
...
estados_t reparar_edificio(Jugador* jugador, ABB<Edificio *>& arbol, Mapa*& mapa){
    ...
    jugador->decrementar_energia(costo_energia);
    cout << endl << MSJ_REPARACION_EXITOSA << endl;
    return st;
}
...
estados_t comprar_bombas(Jugador* jugador)
{
    ...
    if((st = obtener_valor_cantidad(bombas, cantidad_minima, cantidad_maxima)) != ST_OK) return st;
    ...
    jugador-> decrementar_energia(costo_energia);
    cout << endl << MSJ_COMPRA_BOMBAS_EXITOSA << endl << endl;
...
{
    cout << endl << MSJ_MOSTRAR_INVENTARIO << endl;
